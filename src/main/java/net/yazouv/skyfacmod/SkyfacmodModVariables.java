...
public class SkyfacmodModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "skyfacmod_mapvars";
		...
		public void readFromNBT(NBTTagCompound nbt) {
			Lucky = nbt.getBoolean("Lucky");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setBoolean("Lucky", Lucky);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				SkyfacmodMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				SkyfacmodMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "skyfacmod_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				SkyfacmodMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				SkyfacmodMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					SkyfacmodMod.PACKET_HANDLER.sendToAll(message);
				else
					SkyfacmodMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
